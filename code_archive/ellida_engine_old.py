...
class EllidaEngine(Process):
    ...
    def __network_setup(self):
        ...
        self.engine_socket = self.context.socket(zmq.PAIR)
        self.manager_socket = self.context.socket(zmq.PAIR)
        self.daemon_socket = self.context.socket(zmq.PAIR)
        self.engine_socket.bind("tcp://*:%s" % EllidaSettings.ENGINE_SOCKET)
        self.manager_socket.bind("tcp://*:%s" % EllidaSettings.MANAGER_SOCKET)
        self.engine_socket.bind("tcp://*:%s" % EllidaSettings.DAEMON_SOCKET)
